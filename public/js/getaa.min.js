...
var getAA=function(){function e(e,t){var a={lat:e,lng:t},n=i(),s=A.getElementById("change-location"),l=A.getElementById("landing-card"),o=A.getElementById("close-landing-card");q.setPosition(a),q.setContent("You are here."),B.setCenter(a),l.style.display="none",A.getElementById("meetings-manager").style.display="inline",A.getElementById("report-error").style.display="inline",A.getElementById("rack-logo").style.display="inline",A.getElementById("btnMenu").style.display="inline",s.style.display="inline",getAA.initMenu(n),k(e,t,n),s.addEventListener("click",function(){l.style.display="inline"}),o.addEventListener("click",function(){l.style.display="none"})}function t(e){for(var t=0,a=e.length;a>t;t++)e[t].classList.remove("day-selected"),e[t].classList.add("day-unselected")}function a(e){for(var a=0,n=e.length;n>a;a++)e[a].addEventListener("click",function(){var a=this.id;if(this.classList.contains("day-unselected")){s("Fetching Meetings..."),g();var n=r(a);L(n)}t(e),this.classList.remove("day-unselected"),this.classList.add("day-selected")})}function n(e,t,a){var n=A.querySelector("#toast");t||(t=""),a||(a=2750);var s={message:e+t,timeout:a};n.MaterialSnackbar.showSnackbar(s)}function s(e){var t=A.querySelector("#toast"),a=A.querySelector(".mdl-snackbar__text");a.innerHTML=e,t.classList.add("mdl-snackbar--active")}function l(e){setTimeout(function(){var e=A.getElementById("toast");e.classList.remove("mdl-snackbar--active")},D*e)}function o(e){for(var t=0;t<C.length;t++)C[t].setMap(null);1==e&&(C=[])}function i(){var e=new Date,t=new Array(7);t[0]="sunday",t[1]="monday",t[2]="tuesday",t[3]="wednesday",t[4]="thursday",t[5]="friday",t[6]="saturday";var a=t[e.getDay()];return a}function r(e){for(var t=[],a=0;a<O.length;a++)O[a].val().day==e&&t.push(O[a]);return t}function d(e,t){window.setTimeout(function(){c(e);var t=new google.maps.LatLng(e.val().latitude,e.val().longitude),a=!1,n=!1,s=!1,l='<span class="bold caps">'+e.val().name+"<br>"+e.val().hour+e.val().min+" "+e.val().timeframe+'</span><br><span class="cap"><a href="http://maps.google.com/?q='+e.val().street+" "+e.val().city+" "+e.val().state+" "+e.val().zip+'">'+e.val().street+"<br>"+e.val().city+", "+e.val().state+"  "+e.val().zip+"</a><br>"+e.val().siteNotes+"</span>";switch(e.val().timeframe){case"am":a=!0;break;case"noon":n=!0;break;case"pm":s=!0}var o=new google.maps.Marker({position:t,map:B,day:e.val().day,meetingOpen:e.val().meetingOpen,meetingClosed:!e.val().meetingOpen,onlyMen:e.val().onlyMen,onlyWomen:e.val().onlyWomen,childcare:e.val().childcare,meditation:e.val().meditation,speaker:e.val().speaker,step:e.val().step,spanish:e.val().spanish,bigBook:e.val().bigBook,discussion:e.val().discussion,tradition:e.val().tradition,beginner:e.val().beginner,am:a,noon:n,pm:s,animation:google.maps.Animation.DROP});C.push(o),google.maps.event.addListener(o,"click",function(e,t){return function(){q.setContent(l),q.open(B,e)}}(o,e))},t)}function c(e){e.val().meetingOpen&&(j.dataset.badge=Number(j.dataset.badge)+1),e.val().meetingOpen||(K.dataset.badge=Number(K.dataset.badge)+1),e.val().onlyMen&&(U.dataset.badge=Number(U.dataset.badge)+1),e.val().onlyWomen&&(J.dataset.badge=Number(J.dataset.badge)+1),e.val().childcare&&(Q.dataset.badge=Number(Q.dataset.badge)+1),e.val().meditation&&(V.dataset.badge=Number(V.dataset.badge)+1),e.val().speaker&&(X.dataset.badge=Number(X.dataset.badge)+1),e.val().step&&($.dataset.badge=Number($.dataset.badge)+1),e.val().spanish&&(ee.dataset.badge=Number(ee.dataset.badge)+1),e.val().bigBook&&(te.dataset.badge=Number(te.dataset.badge)+1),e.val().discussion&&(ae.dataset.badge=Number(ae.dataset.badge)+1),e.val().tradition&&(ne.dataset.badge=Number(ne.dataset.badge)+1),e.val().beginner&&(se.dataset.badge=Number(se.dataset.badge)+1),"am"===e.val().timeframe&&(le.dataset.badge=Number(le.dataset.badge)+1),"noon"===e.val().timeframe&&(oe.dataset.badge=Number(oe.dataset.badge)+1),"pm"===e.val().timeframe&&(ie.dataset.badge=Number(ie.dataset.badge)+1),e.val().name&&(re.dataset.badge=Number(re.dataset.badge)+1)}function g(){for(var e=0,t=de.length;t>e;e++)de[e].dataset.badge="0"}function u(){for(var e=0,t=de.length;t>e;e++)de[e].MaterialCheckbox.check();for(var a=A.querySelector(".day-selected").id,e=0,n=C.length;n>e;e++)C[e].day===a&&C[e].setMap(B)}function y(){for(var e=0,t=de.length;t>e;e++)de[e].MaterialCheckbox.uncheck();o()}function m(){Me.addEventListener("click",function(){b(Ne.value)?v(Ne.value):n("Address contains invalid characters")}),Ne.onkeypress=function(e){var t=e||window.event,a=t.which||t.keyCode;"13"==a&&Me.click()}}function b(e){return/[A-Za-z0-9'\.\-\s\,]/.test(e)}function v(t){var a=new google.maps.Geocoder;a.geocode({address:t},function(t,a){if(a===google.maps.GeocoderStatus.OK){myResult=t[0].geometry.location;var s=myResult.toString().slice(1,-1).split(", "),l=parseFloat(s[0]),o=parseFloat(s[1]);e(l,o)}else n("Location not found.  Please adjust address.  Error: ",a)})}function p(e){for(var t=0,a=e.length;a>t;t++)e[t].addEventListener("click",function(){"every"===this.id?(this.checked&&u(),this.checked||y()):(this.checked&&f("add",this.id.toString()),this.checked||f("remove",this.id.toString()))})}function f(e,t){switch(t){case"openMeeting":t="meetingOpen";break;case"closedMeeting":t="meetingClosed";break;case"mensOnly":t="onlyMen";break;case"womensOnly":t="onlyWomen"}for(var a=0,n=C.length;n>a;a++)if(C[a][t])switch(e){case"add":C[a].setMap(B);break;case"remove":C[a].setMap(null)}}function h(e,t,a){t.setPosition(a),t.setContent(e?"Error: The Geolocation service failed.":"Error: Your browser doesn't support geolocation.")}function k(e,t,a){var n=[],o=M.query({center:[e,t],radius:F});o.on("key_entered",function(e,t){n.push(e)}),o.on("ready",function(){n.length>0?(s("Fetching Meetings..."),S(n).then(function(e){var t=r(a);L(t)},function(e){console.log(e)})):(s('No area meetings found.  You are encouraged to volunteer to add them.  Click <a href="/admin">Meetings Manager</a> to become a site administrator.'),l(2e3))})}function S(e){return Promise.all(e.map(E))}function E(e){var t=N.child(e);return t.once("value").then(function(e){O.push(e)},function(e){n("Error Fethcing Meetings: "+e)})}function L(e){o(!0);for(var t=0;t<e.length;t++)t===e.length-1&&l(t),d(e[t],t*D)}var B,q,w={},I=new Firebase("https://ack.firebaseio.com"),M=new GeoFire(I.child("geofire")),N=I.child("meetings"),C=[],O=[],A=document,D=10,F=96,P=A.getElementById("menu"),T=A.querySelector(".actionButtonTop"),W=A.querySelector(".menuIcon"),z=A.querySelector("#sunday"),G=A.querySelector("#monday"),R=A.querySelector("#tuesday"),x=A.querySelector("#wednesday"),H=A.querySelector("#thursday"),Y=A.querySelector("#friday"),_=A.querySelector("#saturday"),Z=[z,G,R,x,H,Y,_];T.addEventListener("click",function(){P.classList.toggle("menuHidden"),P.classList.toggle("menuShown"),W.classList.toggle("flip"),W.classList.toggle("menuIconDown"),W.classList.toggle("menuIconUp")}),w.initMap=function(){B=new google.maps.Map(A.getElementById("map"),{zoom:12,minZoom:10,mapTypeControl:!1}),q=new google.maps.InfoWindow({}),navigator.geolocation?navigator.geolocation.getCurrentPosition(function(t){lat=t.coords.latitude,lng=t.coords.longitude,e(lat,lng)},function(){h(!0,q,B.getCenter())}):h(!1,q,B.getCenter())},w.initMenu=function(e){switch(t(Z),g(),e){case"sunday":z.classList.toggle("day-unselected"),z.classList.toggle("day-selected");break;case"monday":G.classList.toggle("day-unselected"),G.classList.toggle("day-selected");break;case"tuesday":R.classList.toggle("day-unselected"),R.classList.toggle("day-selected");break;case"wednesday":x.classList.toggle("day-unselected"),x.classList.toggle("day-selected");break;case"thursday":H.classList.toggle("day-unselected"),H.classList.toggle("day-selected");break;case"friday":Y.classList.toggle("day-unselected"),Y.classList.toggle("day-selected");break;case"saturday":_.classList.toggle("day-unselected"),_.classList.toggle("day-selected")}a(Z),W.className+=" menuIconDown"};var j=A.getElementById("lblOpenMeeting"),K=A.getElementById("lblClosedMeeting"),U=A.getElementById("lblMensOnly"),J=A.getElementById("lblWomensOnly"),Q=A.getElementById("lblChildcare"),V=A.getElementById("lblMeditation"),X=A.getElementById("lblSpeaker"),$=A.getElementById("lblStep"),ee=A.getElementById("lblSpanish"),te=A.getElementById("lblBigBook"),ae=A.getElementById("lblDiscussion"),ne=A.getElementById("lblTradition"),se=A.getElementById("lblBeginner"),le=A.getElementById("lblAm"),oe=A.getElementById("lblNoon"),ie=A.getElementById("lblPm"),re=A.getElementById("lblEvery"),de=[j,K,U,J,Q,V,X,$,ee,te,ae,ne,se,le,oe,ie,re],ce=A.querySelector("#openMeeting"),ge=A.querySelector("#closedMeeting"),ue=A.querySelector("#mensOnly"),ye=A.querySelector("#womensOnly"),me=A.querySelector("#childcare"),be=A.querySelector("#meditation"),ve=A.querySelector("#speaker"),pe=A.querySelector("#step"),fe=A.querySelector("#spanish"),he=A.querySelector("#bigBook"),ke=A.querySelector("#discussion"),Se=A.querySelector("#tradition"),Ee=A.querySelector("#beginner"),Le=A.querySelector("#am"),Be=A.querySelector("#noon"),qe=A.querySelector("#pm"),we=A.querySelector("#every"),Ie=[ce,ge,ue,ye,me,be,ve,pe,fe,he,ke,Se,Ee,Le,Be,qe,we],Me=A.getElementById("locate-submit"),Ne=A.querySelector("#manual-location");return A.addEventListener("DOMContentLoaded",function(e){window.snackbarContainer=A.querySelector("#toast"),window.componentHandler.upgradeAllRegistered(),p(Ie),m()}),w}();
